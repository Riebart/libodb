...
/// The BankDS datastore works around the principle of approximately contiguous placement of objects in memory.
///
/// Unlike a linked list (such as LinkedListDS) where data can be scattered across multiple memory pages, BankDS attempts maintain locality by allocating large blocks of contiguous memory and copying new items into those blocks.
/// While this is typically the job of malloc (or some other memory allocation method), malloc incurs both time and space overhead for each alloc. By reducing the number of allocs required and handling the pointer arithmetic internally these overheads are efficiently culled from the operations required.
///
/// Note that since BankDS is locally contiguous it is possible to index into it, much like an array. This incurs some overhead, but it is possible and is provided through the BankDS::get_at function.
/// Since it is not possible to free only a piece of each block deletions are handled by BankDS by pushing the memory locations of 'deleted' items onto a stack.
/// At the beginning of each insertion (more accurately: addition since most of the time items are being added to the end of the datastore) this stack is checked for contents. If the stack is empty then the new item is appended to the end, however if the stack contains any memory locations, they new data is copied to the top memory location. The stack is then popped to 'unmark' that location.
...
public:
    ...
    /// @param [in] datalen The length (in bytes) of each unit of data.
...
protected:
    ...
    /// The number of bytes per bucket. Equal to cap*datalen.
    uint64_t cap_size;
...
/// A datastore backend that does not contain data, but rather pointers to data contained in a direct datastore.
///
/// BankIDS uses the same structure as BankDS but, instead of storing the actual data, it stores the pointers to data that resides elsewhere in memory. By overriding the BankDS::add_element and BankDS::get_at to add a single operation before the base versions the indirection is achieved with minimal code.
class BankIDS : public BankDS
{
public:
    /// Unique constructor for a BankIDS object.
    /// Since BankIDS requires only two parameters (since datalen is automatically set to sizeof(char*)), both of which can be default, only one constructor is needed.
    /// @param [in] cap The number of items to store in each block. (The default value is 100,000 as it has shown to have very good performance)
    /// @param [in] parent A pointer to the DataStore that spawned this one through a BankDS::clone operation. This parameter will rarely be supplied by the user.
...
protected:
    /// Add a piece of raw data to the datastore.
    /// This function adds a single address-of (&) operation to perform the necessary indirection on the input.
    /// @param [in] rawdata A pointer to the the location of the data to be added.
    /// @return A pointer to the location of the added data in the datastore. By returning a pointer this reduces the lookup overhead to a minimal level.
    virtual void* add_element(void* rawdata);
