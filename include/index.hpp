...
/// Class container for giving the user access to a secure piece of data to manually add to index tables and index groups.
/// By keeping all members private it is able to give the user the ability to pass around a piece of data that cannot be tampered with but still allows for manual insertion into index tables or index groups. Because anything that is intended to be able to access a DataObj instance is declared as a friend (ODB, Index, IndexGroup), and thus privileged, everything is private and no accessors or settors are provided.
///
/// Since all accesses must be privileged, any class that uses this object must be a friend.
class DataObj
{
...
/// Class that takes care of grouping index tables into more abstract logical structures.
/// By allowing for groups of index tables to be made more abstract structures and relations can be represented. Further, IndexGroups support nesting IndexGroups for further abstraction.
/// This means that a uniform interface can be presented to any machine learning or artificial intelligence systems placed on top of this backend.
class IndexGroup
{
    /// Allows ODB objects to call the IndexGroup::add_data_v method directly and skip the overhead of verifying data integrity since that is guaranteed in that usage scenario.
    friend class ODB;
...
public:
    ...
    /// @param condition A condition function that returns true if the piece of data 'passes' (and should be added to the query results) and false if the data fails (and will not be returned in the results).
    ...
    /// @todo Add a recursive flavour. It will have to return the number of items in all contained indices though. Is there a way to stop before indices at the last layer of IndexGroups?
...
protected:
    ...
    /// @param [in] ds A pointer to a datastore that will be filled with the results of the query.
...
private:
    ...
    /// An expanding list of pointers to the IndexGroups contained in this group. These are iterated over whenever data is added or a query is run.
    /// @todo Move to something homegrown?
    vector<IndexGroup*> indices;
};

/// Superclass for all specific implementations of indices (linked lists, redblack trees, tries, etc).
...
public:
    ...
    /// Index implementations starting with "Keyed" are key-value index tables, all others are value-only index tables. Note that a value-only index table has less storage overhead but must perform comparisons directly on the data and may result in more complicated compare functions. Key-value index tables however require a keygen function that generates a key from a piece of data.
